...
"""
Construct a graph based on the sample features
"""
def getGraph(dataset_str, features, L, K, method,data_name):
    print(method)
    if method == 'pearson':
        ...
        if os.path.exists(NE_path):
            ...
            in_matrix[np.isnan(in_matrix)] = 0
            in_matrix[np.isinf(in_matrix)] = 0
    ...
    pd.DataFrame(mat_K).to_csv('./data/'+dataset_str+'/graphmatrix_'+data_name+'.csv', index=False)
    

    return mat_K
    
"""
Load RNA-seq data set and perfrom preprocessing
"""
def load_data(data_path, dataset_str, data_name,is_NE=True, n_clusters=20, K=None):
    # Get data
    DATA_PATH = data_path
    ...
    if(dataset_str == "ANM1" or dataset_str == "ANM2"):
        features = features.T
    ...
    if is_NE:
        ...
        method = 'pearson'
    adj = getGraph(dataset_str, features, L, K, method,data_name)
    
    return adj, features, cells